...
export default function Payments() {
 const [activeTab, setActiveTab] = useState("p")
  ...
  const handlePayment = async (pid, driver) => {
  setSelectedDriver(driver);
  setShowPaymentModal(true);

  try {
    const response = await api.patch(`/api/pay/${pid}/`);

   
  } catch (err) {
    console.error(err.response?.data?.detail || "Error updating payment");
  } 
};
              ...
              {filteredPayments.length > 0 ? (
                filteredPayments.map((payment) => (
                  <tr key={payment.id}>
                    ...
                    <td>{Number(payment.amount)} ETB</td>
